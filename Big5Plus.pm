...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Ebig5plus::uc($char[$i]) ne Ebig5plus::lc($char[$i]))) {
            $char[$i] = '[' . Ebig5plus::uc($char[$i]) . Ebig5plus::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Big5Plus/Big5Plus/JA.pm
...
Definitions in Ebig5plus.pm.
  ...
  @{Ebig5plus::eV}             qr{(?:[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE\x0C\x0A\x0D])}
