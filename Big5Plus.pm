...
# functions of package Ebig5plus
    ...
    elsif (m{\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ebig5plus::fc';           }
    ...
    elsif (m{\G \b fc \b         (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Ebig5plus::fc_';               }
    elsif (m{\G    (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
...
# functions of package Ebig5plus
        ...
        elsif ($string =~ m{\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ebig5plus::fc';      $slash = 'm//'; }
        ...
        elsif ($string =~ m{\G \b fc \b                                              }oxgc) { $e_string .= 'Ebig5plus::fc_';               $slash = 'm//'; }
        elsif ($string =~ m{\G    (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Ebig5plus::uc($char[$i]) ne Ebig5plus::fc($char[$i]))) {
...
sub e_use_noimport {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (open($fh, $realfilename)) {
            ...
            if ($script =~ m/^ \s* use \s+ Big5Plus \s* ([^\x81-\xFE;]*) ; \s* \n? $/oxms) {
