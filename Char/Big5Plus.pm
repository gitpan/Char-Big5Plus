...
######################################################################
#
# Char::Big5Plus - Source code filter to escape Big5Plus script
#
# http://search.cpan.org/dist/Char-Char::Big5Plus/
#
...
BEGIN { CORE::require Char::Ebig5plus; }
...
my $function_getc;        # getc    to getc    or Char::Big5Plus::getc
...
sub import {
    ...
    if (Char::Ebig5plus::e("$filename.e")) {
        if (exists $ENV{'SJIS_DEBUG'}) {
            ...
            my $mtime_filter = (Char::Ebig5plus::stat(__FILE__     ))[9];
            my $mtime_source = (Char::Ebig5plus::stat($filename    ))[9];
            my $mtime_escape = (Char::Ebig5plus::stat("$filename.e"))[9];
    ...
    if (not Char::Ebig5plus::e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Ebig5plus::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
    ...
    my $fh = gensym();
    Char::Ebig5plus::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::Big5Plus::escape_script {
    ...
    my $fh = gensym();
    Char::Ebig5plus::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Ebig5plus(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Ebig5plus %s.0;\n", $Char::Big5Plus::VERSION); # require run-time routines version

        # use Char::Big5Plus version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::Big5Plus \s* ([^\x81-\xFE;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::Big5Plus::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/Big5Plus.pm}} @INC;
...
****************************************************
              ...
              CONFLICT Char/Big5Plus.pm FILE
...
END
                    ...
                    die "Script $0 expects Char/Big5Plus.pm $version, but @{[__FILE__]} is version $Char::Big5Plus::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Ebig5plus.pm $version, but \\\$Char::Ebig5plus::VERSION is \$Char::Ebig5plus::VERSION" if \$Char::Ebig5plus::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::Big5Plus::VERSION) {
                    die "Script $0 required Char/Big5Plus.pm $version, but @{[__FILE__]} is only version $Char::Big5Plus::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::Big5Plus::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::Big5Plus::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::Big5Plus::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::Big5Plus::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                                              2   3                                 4   5
                q<eval{local $Char::Big5Plus::re_t=%s; local $Char::Big5Plus::re_a=''; while($Char::Big5Plus::re_t =~ %s){%s local $^W=0; local $Char::Big5Plus::re_r=%s; %s$Char::Big5Plus::re_t="$Char::Big5Plus::re_a${1}$Char::Big5Plus::re_r$'"; pos($Char::Big5Plus::re_t)=length "$Char::Big5Plus::re_a${1}$Char::Big5Plus::re_r"; $Char::Big5Plus::re_a=substr($Char::Big5Plus::re_t,0,pos($Char::Big5Plus::re_t)); } return $Char::Big5Plus::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::Big5Plus::re_r=eval $Char::Big5Plus::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::Big5Plus::re_t=%s; while($Char::Big5Plus::re_t =~ %s){%s local $^W=0; local $Char::Big5Plus::re_r=%s; %s$Char::Big5Plus::re_t="$`$Char::Big5Plus::re_r$'"; pos($Char::Big5Plus::re_t)=length "$`$Char::Big5Plus::re_r"; } return $Char::Big5Plus::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::Big5Plus::re_r=%s; %s"%s$Char::Big5Plus::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $Char::Big5Plus::re_n=0; local $Char::Big5Plus::re_a=''; while(%s =~ %s){%s local $^W=0; local $Char::Big5Plus::re_r=%s; %s%s="$Char::Big5Plus::re_a${1}$Char::Big5Plus::re_r$'"; pos(%s)=length "$Char::Big5Plus::re_a${1}$Char::Big5Plus::re_r"; $Char::Big5Plus::re_a=substr(%s,0,pos(%s)); $Char::Big5Plus::re_n++} return %s$Char::Big5Plus::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::Big5Plus::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::Big5Plus::re_r=%s; %s%s="$`$Char::Big5Plus::re_r$'"; pos(%s)=length "$`$Char::Big5Plus::re_r"; $Char::Big5Plus::re_n++} return %s$Char::Big5Plus::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::Big5Plus::re_r=%s; %s%s="%s$Char::Big5Plus::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::Big5Plus::re_r=%s; %s%s="%s$Char::Big5Plus::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::Big5Plus::re_r=eval $Char::Big5Plus::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/Big5Plus.pm and Char/Ebig5plus.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::Big5Plus;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::Big5Plus;
  use Char::Big5Plus ver.sion;             --- require minimum version
  use Char::Big5Plus ver.sion.0;           --- expects version (match or die)
  use Char::Big5Plus qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use Char::Big5Plus ver.sion qw(ord reverse getc);
  use Char::Big5Plus ver.sion.0 qw(ord reverse getc);
...
Char::Big5Plus software is "middleware" between perl interpreter and your Perl script
written in Big5Plus.
...
By "use Char::Big5Plus;", it automatically interpret your script as Big5Plus. The various
...
import of Char/Ebig5plus.pm works well.

   in Char/Ebig5plus.pm
   #
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char/Big5Plus.pm provides)
...
=head1 Escaping Second Octet (Char/Big5Plus.pm provides)
...
=head1 Multiple-Octet Character Regular Expression (Char/Big5Plus.pm provides)

Char/Big5Plus.pm clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Char::Ebig5plus::ignorecase()' (Char/Big5Plus.pm provides)
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  $_ =~                   ($_ =~ m/ditto$Char::Ebig5plus::matched/) ?
  s/regexp/replacement/   eval{ Char::Ebig5plus::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Ebig5plus::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Char::Ebig5plus::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Char::Ebig5plus::matched/) ?
  s/regexp/replacement/b  eval{ Char::Ebig5plus::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
Char/Big5Plus.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
Insert 'Char::Ebig5plus::' instead of '-' of operator.
  ...
  --------------------------------------------------------------------------
  -r       Char::Ebig5plus::r   File or directory is readable by this (effective) user or group
  -w       Char::Ebig5plus::w   File or directory is writable by this (effective) user or group
  -e       Char::Ebig5plus::e   File or directory name exists
  -x       Char::Ebig5plus::x   File or directory is executable by this (effective) user or group
  -z       Char::Ebig5plus::z   File exists and has zero size (always false for directories)
  -f       Char::Ebig5plus::f   Entry is a plain file
  -d       Char::Ebig5plus::d   Entry is a directory
  -t       -t         The filehandle is a TTY (as reported by the isatty() system function;
                      filenames can't be tested by this test)
  -T       Char::Ebig5plus::T   File looks like a "text" file
  -B       Char::Ebig5plus::B   File looks like a "binary" file
  -M       Char::Ebig5plus::M   Modification age (measured in days)
  -A       Char::Ebig5plus::A   Access age (measured in days)
  -C       Char::Ebig5plus::C   Inode-modification age (measured in days)
  -s       Char::Ebig5plus::s   File or directory exists and has nonzero size
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -R       Char::Ebig5plus::R   File or directory is readable by this real user or group
  -W       Char::Ebig5plus::W   File or directory is writable by this real user or group
  -X       Char::Ebig5plus::X   File or directory is executable by this real user or group
  -l       Char::Ebig5plus::l   Entry is a symbolic link
  -S       Char::Ebig5plus::S   Entry is a socket
  --------------------------------------------------------------------------
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -o       Char::Ebig5plus::o   File or directory is owned by this (effective) user
  -O       Char::Ebig5plus::O   File or directory is owned by this real user
  -p       Char::Ebig5plus::p   Entry is a named pipe (a "fifo")
  -b       Char::Ebig5plus::b   Entry is a block-special file (like a mountable disk)
  -c       Char::Ebig5plus::c   Entry is a character-special file (like an I/O device)
  -u       Char::Ebig5plus::u   File or directory is setuid
  -g       Char::Ebig5plus::g   File or directory is setgid
  -k       Char::Ebig5plus::k   File or directory has the sticky bit set
  --------------------------------------------------------------------------
...
=item * Returns Next Character
  ...
  $getc = Char::Big5Plus::getc(FILEHANDLE);
  $getc = Char::Big5Plus::getc($filehandle);
  $getc = Char::Big5Plus::getc;
  ...
  If you import getc "use Char::Big5Plus qw(getc);", getc of your script will be rewritten
  in Char::Big5Plus::getc. Char::Big5Plus::getc is not compatible with getc of JPerl.
...
=item * Length by Big5Plus Character
  ...
  of the scalar value $string. If $string is omitted, it returns the Char::Big5Plus::length
  of $_.

  Do not try to use Char::Big5Plus::length to find the size of an array or hash. Use scalar
...
=item * Substr by Big5Plus Character
  ...
  my $color  = Char::Big5Plus::substr $s, 4, 5;      # black
  my $middle = Char::Big5Plus::substr $s, 4, -11;    # black cat climbed the
  my $end    = Char::Big5Plus::substr $s, 14;        # climbed the green tree
  my $tail   = Char::Big5Plus::substr $s, -4;        # tree
  my $z      = Char::Big5Plus::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the Char::Big5Plus::substr() subroutine as an
  ...
  end of the string, Char::Big5Plus::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  Char::Big5Plus::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = Char::Big5Plus::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = Char::Big5Plus::substr $name, 7;      # returns undef, with warning
  Char::Big5Plus::substr($name, 7) = 'gap';        # raises an exception
  ...
  my $s = "The black cat climbed the green tree";
  my $z = Char::Big5Plus::substr $s, 14, 7, "jumped from";    # climbed
  # $s is now "The black cat jumped from the green tree"

  Note that the lvalue returned by the three-argument version of Char::Big5Plus::substr() acts
  ...
  $x = '1234';
  for (Char::Big5Plus::substr($x,1,2)) {
      $_ = 'a';   print $x,"\n";    # prints 1a4
      $_ = 'xyz'; print $x,"\n";    # prints 1xyz4
      $x = '56789';
      $_ = 'pq';  print $x,"\n";    # prints 5pq9
  ...
  $x = '1234';
  for (Char::Big5Plus::substr($x, -3, 2)) {
      $_ = 'a';   print $x,"\n";    # prints 1a4, as above
      $x = 'abcdefg';
      print $_,"\n";                # prints f
  }
...
=item * Rindex by Big5Plus Character
  ...
  $rindex = Char::Big5Plus::rindex($string,$substring,$offset);
  $rindex = Char::Big5Plus::rindex($string,$substring);

  This subroutine works just like Char::Big5Plus::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse Char::Big5Plus::index).
...
=item * Filename Globbing
  ...
  This operator is implemented via the Char::Ebig5plus::glob() subroutine. See Char::Ebig5plus::glob
  of Char/Ebig5plus.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use Char::Big5Plus qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use Char::Big5Plus qw(reverse);" exists or not.
...
=head1 Un-Escaping bytes::* Subroutines (Char/Big5Plus.pm provides)

Char/Big5Plus.pm removes 'bytes::' at head of subroutine name.
...
If Perl version is older than 5.14, Char::Big5Plus::substr differs from CORE::substr, and
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Char::Ebig5plus::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${`}            Char::Ebig5plus::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Char::Ebig5plus::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Char::Ebig5plus::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Char::Ebig5plus::MATCH()       $1
  ${&}            Char::Ebig5plus::MATCH()       $1
  $MATCH          Char::Ebig5plus::MATCH()       $1
  ${^MATCH}       Char::Ebig5plus::MATCH()       $1
  $'              $'                   $'
  ${'}            ${'}                 $'
  $POSTMATCH      Char::Ebig5plus::POSTMATCH()   $'
  ${^POSTMATCH}   Char::Ebig5plus::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
Even if ${^WIN32_SLOPPY_STAT} is set to a true value, file test functions Char::Ebig5plus::*(),
Char::Ebig5plus::lstat(), and Char::Ebig5plus::stat() on Microsoft Windows open the file for the path
...
exist at time of be written this document and JPerl did exist.
                        ...
                        (a)     (b)     (c)     (d)     (e)
                                      JPerl,japerl    Encode,Char::Big5Plus
      +--------------+-------+-------+-------+-------+-------+
      | data         |  Old  |  Old  |  New  |  Old  |  New  |
      +--------------+-------+-------+-------+-------+-------+
      | script       |  Old  |      Old      |      New      |
...
Char::Big5Plus software remains one language and one interpreter.
