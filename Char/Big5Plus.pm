...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                for (@list) {
                    $Char::Ebig5plus::function_ord     = 'Char::Big5Plus::ord'     if /\A ord \z/oxms;
                    $Char::Ebig5plus::function_ord_    = 'Char::Big5Plus::ord_'    if /\A ord \z/oxms;
                    $Char::Ebig5plus::function_reverse = 'Char::Big5Plus::reverse' if /\A reverse \z/oxms;
                    $Char::Ebig5plus::function_getc    = 'Char::Big5Plus::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Char::Big5Plus::escape();
...
=head1 Escaping Multiple-Octet Code (Char::Big5Plus software provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char::Big5Plus software provides)

Char::Big5Plus software applies multiple-octet anchoring at beginning of regular expression.
...
=head1 Escaping Second Octet (Char::Big5Plus software provides)

Char::Big5Plus software escapes second octet of multiple-octet character in regular
expression.
...
=head1 Multiple-Octet Character Regular Expression (Char::Big5Plus software provides)

Char::Big5Plus software clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Char::Ebig5plus::ignorecase()' (Char::Big5Plus software provides)

Char::Big5Plus software applies calling 'Char::Ebig5plus::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   CORE::eval{ Char::Ebig5plus::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Ebig5plus::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Char::Ebig5plus::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  CORE::eval{ Char::Ebig5plus::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (Char::Big5Plus software provides)

Char::Big5Plus software removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char::Big5Plus software provide)
...
=head1 Escaping File Test Operators (Char::Big5Plus software provide)
...
=item * Eval Big5Plus Script
  ...
  $eval = Char::Big5Plus::eval { block };
  $eval = Char::Big5Plus::eval $expr;
  $eval = Char::Big5Plus::eval;

  The Char::Big5Plus::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, Char::Big5Plus::eval { block }
  and Char::Big5Plus::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, Char::Big5Plus::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of Char::Big5Plus::eval, the value returned from an Char::Big5Plus::eval is
  ...
  Char::Big5Plus::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  Char::Big5Plus::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      Char::Big5Plus::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The Char::Big5Plus::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow Char::Big5Plus::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  Char::Big5Plus::eval cannot trap syntax errors.

  The Char::Big5Plus::eval $expr form can trap syntax errors because it parses the code
  ...
  after the Char::Big5Plus::eval is complete, as do any subroutine or format definitions.
  The code of the Char::Big5Plus::eval is treated as a block, so any locally scoped
  variables declared within the Char::Big5Plus::eval last only until the Char::Big5Plus::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  Char::Big5Plus::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  Char::Big5Plus::eval { block }    eval { block }
  Char::Big5Plus::eval $expr        eval Char::Big5Plus::escape $expr
  Char::Big5Plus::eval              eval Char::Big5Plus::escape
  -------------------------------------------------

  To tell the truth, the subroutine Char::Big5Plus::eval does not exist. If it exists,
  you will troubled, when Char::Big5Plus::eval has a parameter that is single quoted
  string included my variables. Char::Big5Plus::escape is a subroutine that makes Perl
  ...
  Since Char::Big5Plus::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because Char::Big5Plus::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  Char::Big5Plus::eval $expr and Char::Big5Plus::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      Char::Big5Plus::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      Char::Big5Plus::eval { $answer = }; # WRONG

      # a runtime syntax error
      Char::Big5Plus::eval '$answer =';   # sets $@
  ...
  Using the Char::Big5Plus::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      Char::Big5Plus::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          Char::Big5Plus::eval { die "foo lives here" };
  ...
  With an Char::Big5Plus::eval, you should be especially careful to remember what's being
  looked at when:

      Char::Big5Plus::eval $x;        # CASE 1
      Char::Big5Plus::eval "$x";      # CASE 2

      Char::Big5Plus::eval '$x';      # CASE 3
      Char::Big5Plus::eval { $x };    # CASE 4

      Char::Big5Plus::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  a temporary is required if you want to mask some but not all errors:
      ...
      # alter $@ on nefarious repugnancy only
      {
          my $e;
          {
              ...
              Char::Big5Plus::eval { test_repugnancy() };
  ...
  The block of Char::Big5Plus::eval { block } does not count as a loop, so the loop
...
=head1 Un-Escaping bytes::* Subroutines (Char::Big5Plus software provides)

Char::Big5Plus software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in Char::Big5Plus software and
