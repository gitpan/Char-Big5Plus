...
sub Char::Big5Plus::escape_script {
    ...
    if (m/^ use Char::Ebig5plus(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        if (s/^ \s* use \s+ Char::Big5Plus \s* ([^;]*) ; \s* \n? $//oxms) {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and (Char::Ebig5plus::uc($char[$i]) ne Char::Ebig5plus::lc($char[$i]))) {
...
=item * Only Char/Big5Plus.pm and Char/Ebig5plus.pm, other modules are unnecessary
...
=head1 Perl5.6 Emulation on perl5.005
  ...
  --------------------------------------------------------------------
  Before          After                  in BEGIN { } of Char/Ebig5plus.pm
  --------------------------------------------------------------------
  binmode(...);   Char::Ebig5plus::binmode(...);   *CORE::GLOBAL::binmode = ...
  open(...);      Char::Ebig5plus::open(...);      *CORE::GLOBAL::open    = ...
  --------------------------------------------------------------------
...
=item * binmode (Perl5.6 emulation on perl5.005)
  ...
  * two arguments
  ...
  If you are using perl5.005, Char::Big5Plus software emulate perl5.6's binmode function.
...
=item * open (Perl5.6 emulation on perl5.005)
  ...
  * three arguments
  ...
  open(FH, "<:crlf:encoding(Char::Big5Plus)", $path) or die "can't open $path: $!";
...
=head1 Ignore utf8 pragma
  ...
  ---------------------------------------------------------------------
  use utf8;       # use utf8;            Char/Ebig5plus.pm provides utf8::*
  no utf8;        # no utf8;             functions even if 'no utf8;'
  use bytes;      # use bytes;           Char/Ebig5plus.pm provides bytes::*
