...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (/\A (?:$q_char)*? [*?] /oxms and (my @glob = Char::Ebig5plus::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
#
# Big5Plus fold case lower case without parameter
#
sub Char::Ebig5plus::fc_() {
    my $s = $_;
    return join '', map {defined($fc{$_}) ? $fc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
=item File test function Char::Ebig5plus::X
  ...
  Function and Prototype     Meaning
  ------------------------------------------------------------------------------
  Char::Ebig5plus::s(*), Char::Ebig5plus::s_()   File has nonzero size (returns size in bytes).
  ------------------------------------------------------------------------------
